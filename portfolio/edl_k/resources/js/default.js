...
function gnb(){
	...
	if(windowWidth < 1200) {
		...
		$('.gnb-list > li > a').on('click', function(){
			$('.gnb-list > li').not($(this).parent('li').toggleClass('on')).removeClass('on');
			$('.gnb-list > li > ul').not($(this).next('ul').stop().slideToggle()).slideUp(); /* 20200129 수정 */
		});
		...
		// main 1depth 메뉴 링크
		$('.header > .gnb-wrap > .gnb-list > li:eq(1) > a').attr("href", "contentPopular.html"); /* 20200213 수정 */
		$('.header > .gnb-wrap > .gnb-list > li:eq(4) > a').attr("href", "noticeList.html"); /* 20200213 수정 */
